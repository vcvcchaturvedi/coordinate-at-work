...
(function () {
  window.getMediaElement = function (mediaStream, options) {
    ...
    if (
      !mediaStream.nodeName ||
      (mediaStream.nodeName.toLowerCase() != "audio" &&
        mediaStream.nodeName.toLowerCase() != "video")
    ) {
    ...
    function setWidth() {
      canvas.width =
        options.width || mediaElement.videoWidth || mediaElement.clientWidth; //|| innerWidth / 3;
      canvas.height =
        options.height ||
        mediaElement.videoHeight ||
        mediaElement.clientHeight ||
        (options.onlyAudio == true ? 150 : canvas.width - canvas.width / 4);
    }
    ...
    function drawImage() {
      ...
      if (preparingSnapshot) {
        context.fillText(
          "Taking snapshot in " + takingSnapshotIn + " seconds.",
          canvas.width / 4,
          canvas.height / 2 - 70
        );
      }
    ...
    function screenStateChange(e, skip) {
      if (
        !skip &&
        e.srcElement != canvas &&
        e.srcElement != canvas.parentNode.parentNode
      )
        return;
  ...
  function internalObject(canvas, options) {
    ...
    if (buttons.has("record-audio")) {
      ...
      recordAudio.className =
        "control " +
        (options.toggle.has("record-audio")
    ...
    if (buttons.has("record-video") && !options.onlyAudio) {
      ...
      recordVideo.className =
        "control " +
        (options.toggle.has("record-video")
    ...
    if (buttons.has("volume-slider")) {
      ...
      slider.max = 100;
      slider.value =
        typeof options.volume != "undefined" ? options.volume : 100;
      slider.onchange = function () {
        options.mediaElement.volume =
    ...
    options.mediaElement.volume =
      typeof options.volume != "undefined" ? options.volume : 1;
    ...
    if (buttons.has("full-screen")) {
      ...
      zoom.className =
        "control " +
        (options.toggle.has("zoom-in") ? "zoom-out selected" : "zoom-in");
      ...
      function screenStateChange(e) {
        ...
        mediaElementContainer.style.width =
          (isFullScreeMode ? window.innerWidth : canvas.width + 4) + "px";
        mediaElementContainer.style.display = isFullScreeMode
          ? "block"
          : "inline-block";

        mediaBox.style.height =
          (isFullScreeMode ? window.innerHeight : canvas.height) + "px";
