...
// let RTCPeerConnection00 = window.RTCPeerConnection;
// // }
...
export const RTCPeerConnection = function (options) {
  var w = window;
  ...
  var ishost = options.ishost;
  var share = options.share;

  var initialtrack = options.initialtoken;
  ...
  console.debug(
    "sdp-constraints",
    JSON.stringify(constraints.mandatory, null, "\t")
  );
  ...
  function createAnswer() {
    ...
    peer
      .setRemoteDescription(new RTCSessionDescription(options.offerSDP))
      .then(function () {
        peer
  ...
  return {
    ...
    addTrack: function (stream) {
      ...
      peer.ontrack = function (event) {
        alert("on track");
        var remoteMediaStream = event.streams[1];
        options.onRemoteStream(stream);
        if (dontDuplicate[remoteMediaStream.id]) return;
        dontDuplicate[remoteMediaStream.id] = true;
  ...
  function getIceServers(connection) {
    console.log("Finished RTC Connection...");
...
try {
  chromeVersion = !!navigator.mozGetUserMedia
    ? 0
